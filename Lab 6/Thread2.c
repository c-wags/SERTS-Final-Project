...
osMessageQId mid_PlayMsgQueue;                               // message queue id
osMessageQDef (PlayMsgQueue, MSGQUEUE_OBJECTS, int32_t);     // message queue object

#define MSGQUEUE_OBJECTS 1 // number of Message Queue Objects
osMessageQId mid_PauseMsgQueue;                               // message queue id
osMessageQDef (PauseMsgQueue, MSGQUEUE_OBJECTS, int32_t); 
...
osThreadDef (Playing, osPriorityNormal, 1, 0);                            // thread function
...
int playing = 0; // whether in play state or not	
...
void Init_Thread (void) {
  ...
  if (!tid_Playing) return;
	...
	mid_PlayMsgQueue = osMessageCreate (osMessageQ(PlayMsgQueue), NULL);  // create msg queue
  if (!mid_PlayMsgQueue) {
    ; // Message Queue object not created, handle failure
  }
	mid_PauseMsgQueue = osMessageCreate (osMessageQ(PauseMsgQueue), NULL);  // create msg queue
  if (!mid_PauseMsgQueue) {
    ; // Message Queue object not created, handle failure
  }
}
...
void Thread (void const *argument) {
  ...
  while (1) {
	  ...
	  if(!strcmp(r_data,"P")){
			playing = 1;
			osMessagePut (mid_PauseMsgQueue, 2, osWaitForever); // Send Message
			...
			FILE *f;  // pointer to file type for files on USB device
			...
			if (ustatus == usbOK){
				...
				f = fopen ("Chris OG Rock Music Game Thing Synth Melody.wav","r");// open a file on the USB device
				if (f != NULL) {
...
void Initializing (void) {  // Switch to Initializing when GUI starts up
	...
	if (ustatus == usbOK){
		...
		//UART_send("Init\n",5);
		//char buffer[50];
		//memset(buffer, 0, 50 * (sizeof buffer[0]) ); // fill buffer with 0's
		
		while (ffind ("U0:*.wav", &info) == fsOK) {     // find whatever is in any drive"
			//sprintf(buffer,"\n%s",info.name);
			//UART_send(buffer,50); // send to UART
			//memset(buffer, 0, 50 * (sizeof buffer[0]) ); // clear buffer
			//UART_send(strcat(info.name,"\n"),strlen(info.name)+1);
...
void Indexing (void) {  // Switch to Indexing when pause button pressed or song ends
	LED_Off(1);
	LED_Off(3);
	LED_Off(4);
	LED_On(2);
	...
	//playing = 0;
	osMessagePut (mid_PauseMsgQueue, 1, 0); // Send Message
	while(1) {
	evt = osMessageGet (mid_PauseMsgQueue, osWaitForever); 
				if (evt.value.v == 2) {
					BSP_AUDIO_OUT_SetMute(AUDIO_MUTE_OFF);
					BSP_AUDIO_OUT_Play((uint16_t *)Audio_Buffer1, BUF_LEN);
				}
...
void Playing (void const *argument) {  // Switch to Playing when play button pressed
			LED_Off(2);
			LED_Off(3);
			LED_Off(4);
			LED_On(1);
		...
		int songStarted = 0; // bool for whether the song has been playing or not
		while(1) {
			evt = osMessageGet (mid_PauseMsgQueue, osWaitForever);           // get message
			...
			if(evt.value.v == 2) {
				...
				//if(!feof(f)) {
					...
					osMessagePut (mid_PlayMsgQueue, 2, 0); // Send Message
					osSemaphoreWait(Sem_id,osWaitForever);
		
					// generate data for audio buffer 1
					fread(&Audio_Buffer1,sizeof(int16_t),2048,f);

					osMessagePut (mid_PlayMsgQueue, 1, 0); // Send Message
					osSemaphoreWait(Sem_id,osWaitForever);
				}
...
void    BSP_AUDIO_OUT_TransferComplete_CallBack(void){	
	osEvent evt; // receive object
	evt = osMessageGet (mid_PlayMsgQueue, 0);           // get message
	if (evt.status == osEventMessage) {
